...
export const caseStudy = {
  ...
  testimonial: {
    author: { name: 'Macy Jackson', role: 'Regional Technical Director' },
    content:
...
This project posed a straightforward problem: fetch data from an API, then serve it to a web page to parse into readable data. As we were develooping this solution, we came across some fairly unique issues:
...
>
  John really spent time getting down to the meat and potatoes of what we needed and worked lightnign fast to start providing solutions we could use and test from Day 1. We cannot wait to see the final standalone product.
</Blockquote>
